...
/**************************************************************************************************************************************************/
const int N=13e3+10, inf=1e18+10;
vector<int> g[N];
int n,m,k;
void dijkstra(map<pi, int> &mp)
{
  vi dis(n+1,inf);
  vi p(n+1,-1);
  set<pi> st;
  ...
  while(!st.empty()){
    ...
    for(auto v:g[node]){
      ...
      if(dis[node]+1<dis[v] and true){
        auto itr=st.find({dis[v],v});
        if(itr!=st.end())
          st.erase(it);
        dis[v]=dis[node]+1;
        p[v]=node;
