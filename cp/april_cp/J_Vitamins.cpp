...
void solve(){
  ...
  ans=min(ans,sing[0]+d[1]);
  ans=min(ans,sing[1]+d[2]);
  ans=min(ans,sing[2]+d[0]);
  for(int i=0;i<3;i++){
    ans=min(ans,(i+1<3)?(d[i]+d[i+1]):(d[i]+d[0]));
  }
  ans=min(ans,sing[0]+sing[1]+sing[2]);
  if(ans>=inf){
    cout<<-1<<endl;
    return;
  }
