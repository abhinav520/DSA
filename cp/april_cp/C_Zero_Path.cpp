...
int n, m;
int testcase=1;
// int rec(int i,int j,vv<int> &v, int sum, vv<int> &arr){
//   if(i>n or j>m){
//     return 0;
//   }
//   if(i==n and j==m){
...
//   //   return arr[i][j];
//   // }
//   if(i+1<=n and arr[i+1][j]==-inf) arr[i+1][j]=rec(i+1,j,v,sum+v[i][j],arr);
//   if(j+1<=m and arr[i][j+1]==-inf) arr[i][j+1]=rec(i,j+1,v,sum+v[i][j],arr);
//   return arr[i][j]=(((i+1<=n)?arr[i+1][j]:0) || ((j+1<=m)?arr[i][j+1]:0));
// }

void solve(){
  cin>>n>>m;
  vv<int> v(n+1, vi (m+1));
  // vv<int> arr(n+2, vi (m+2,-inf));
  for(int i=1;i<=n;i++){
    for(int j=1;j<=m;j++){
      cin>>v[i][j];
    }
  ...
  vv<int> mn(n + 1, vi(m + 1)), mx(n + 1, vi(m + 1));
  mn[1][1]=mx[1][1]=v[1][1];
  for(int i=2;i<=n;i++){
    mn[i][1]=mx[i][1]=mx[i-1][1]+v[i][1];
  }
  for(int j=2;j<=m;j++){
    mn[1][j]=mx[1][j]=mx[1][j-1]+v[1][j];
  }
  for(int i=2;i<=n;i++){
    for(int j=2;j<=m;j++){
  ...
  dbg(mx[n][m],mn[n][m]);
  if(mx[n][m]%2==1 or mx[n][m]<0 or mn[n][m]>0){
    no;
  }
  else yes;
}
