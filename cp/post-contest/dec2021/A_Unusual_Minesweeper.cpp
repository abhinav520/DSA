...
int dy[4]={1,-1,1,-1};
const int N=1e9;
void dfs(int v, int* x, int y[], int*t, int vis[], int n, int k, int &mn){
  vis[v]=1;
  mn=min(mn,t[v]);
  dbg(mn,t[v],v);
  int xc=x[v];
  int yc=y[v];
  for(int i=0;i<n;i++){
    if(vis[i]==1 or i==v) continue;
    int r=x[i], c=y[i];
    if((abs(xc-r)<=k and yc==c) or (abs(yc-c)<=k and xc==r)){
      dfs(i, x, y, t, vis, n, k, mn);
    }
  }
  return;
}
