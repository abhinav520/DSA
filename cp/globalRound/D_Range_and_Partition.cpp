...
int n, k;
bool chk(vi &a, vi &pre, bool flag, int x){
  for(int i=1;i<=n-x+1;i++){
    int l=i;
    int r=i+x-1;
    int in_range=pre[r]-pre[l-1];
    int out_range=n-in_range;
    if(in_range-out_range>=k){   //necessary and sufficient condition
      if(flag){
        cout<<l<<" "<<r<<endl;
        int i=1;
        int sum=0;
