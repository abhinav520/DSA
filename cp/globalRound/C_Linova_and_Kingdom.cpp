...
/**************************************************************************************************************************************************/
const int N=2e5+10, inf=-1;
int n, m, sx, sy, ex, ey;
int dx[]={1,0,-1,0};
int dy[]={0,1,0,-1};
char dir[]={'D','R','U','L'};

bool isValid(vector<vector<int>> &g, vector<vector<int>> &lvl, int x, int y)
{
  return (x>=0 and y>=0 and x<n and y<m and lvl[x][y]==inf and g[x][y]==1);
}
...
void solve(){
  ...
  vector<vector<int>> lvl(n, vi(m,inf));
  vector<char> ans;
  queue<pi>q;
  q.push({sx,sy});
  lvl[sx][sy]=0;
