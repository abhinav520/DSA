...
vector<int> GetHIndexScore(vector<int> citations_per_paper)
{
  ...
  int n=1;
  map<int,int> mp;
  int siz=citations_per_paper.size();
  for(int i=0;i<siz;i++){
    if(i>0) mp[citations_per_paper[i]]=mp[citations_per_paper[i-1]];
    mp[citations_per_paper[i]]++;
    h_index.push_back(min(n, mp[citations_per_paper[i]]));
    n++;
  }
  dbg(mp);
  return h_index;
