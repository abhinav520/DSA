...
bool chk(vi &v, int n, int x){
  //find lis of size x (u can also add some integer to obtain lis)
  return false;
}
...
void solve(){
  ...
  while(j<n){
    ...
    spike_flag2=j++;
    dbg(spike_flag1,spike_flag2);
    dbg(j);
    while(j<n and a[j-1]<a[j] and a[spike_flag1-1]+(spike_flag2-spike_flag1)<a[spike_flag2]) j++;
    dbg(j,i);
    len=max(len,j-i-1);
    i=spike_flag1;
    spike_flag1=spike_flag2;
    spike_flag2=j++;
  }
  dbg(len);
  i=0,j=1;
