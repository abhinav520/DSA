...
void solve(){
  ini(n);
  spi sl;   //min l, cost
  set<pi, decltype(&cmp)> sr(cmp);  // max r,cost
  set<pi,decltype(&cmp)> lr(cmp);   //maxlen,cost
  for(int i=0;i<n;i++){
    int l,r,c;
    scan(l,r,c);
    sl.insert({l,c});
    ...
    auto plr=lr.begin();
    dbg(sl,sr,lr);
    int ans=pl->second+pr->second;
    if(pr->first-pl->first+1==plr->first){
      ans=min(ans,plr->second);
    }
    cout<<ans<<endl;
  }
}
