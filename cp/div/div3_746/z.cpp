...
void bfs(vector<vector<int>> &g, vector<int> &lvl, int n, int s){
  vector<int> vis(n);
  queue<int> q;
  q.push(s);
  lvl[s] = 0;
...
void solve(){
  ...
  for(int i=0;i<n-1;i++){
    ...
    int uh=h[i].second;
    int j=i+1;
    while(j<n-1 and h[j].second<uh){
      g[u].push_back(h[j].first);
      g[h[j].first].push_back(u);
      j++;
    }
  ...
  dbg(g);
  vector<int> lvlstart(n,INT32_MAX);
  vector<int> lvlend(n,INT32_MAX);
  bfs(g,lvlstart,n,0);
  bfs(g,lvlend,n,n-1);
  int q;
  cin >> q;
  while (q--)
  {
    ...
    cin >> x;
    x--;
    cout<<min(lvlstart[x],lvlend[x])<<endl;
  }
}
