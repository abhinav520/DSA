...
int n,m,L,s,t,f;
void dijkstra(vi &dis, vector<pi> g[] , vi &p)
{
...
void solve(){
  ...
  fof(i,m){
    ...
    if(c==0){
      ...
      c1=1;
      c2=inf;
      mp2[{min(a,b),max(a,b)}]=c;
    } 
    g[a].push_back({b,c1});
    g[b].push_back({a,c1});
    g2[a].push_back({b,c2});
    g2[b].push_back({a,c2});
    mp[{min(a, b), max(a, b)}] = c2;
  }
  // dbg(mp);
  vi d1(n,inf);
  vi d2(n,inf);
  ...
  for(int i=1;i<sz(path);i++){
    r=path[i];
    dbg(l,r);
    if(mp2.count({min(l,r),max(l,r)})){
      if(f){
        mp[{min(l, r), max(l, r)}] = diff;
        f=false;
      }
      else{
        mp[{min(l, r), max(l, r)}] = 1;
      }
    }
    l=r;
  }
  for(auto [p,q]:mp){
    cout<<p.first<<" "<<p.second<<" "<<q<<endl;
  }
}
