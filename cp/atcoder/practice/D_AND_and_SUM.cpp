...
bool isValid(int x,int y, vv<int> &vis){
  return (x>=0 and x<7 and y>=0 and y<7 and vis[x][y]==0);
}

bool dfs(int x, int y,vv<int> &vis){
  vis[x][y]=1;
  if(x==0 and y==7) return true;
  for(int i=0;i<6;i++){
    if(isValid(x+dx[i],y+dy[i],vis)){
      return dfs(x+dx[i],y+dy[i],vis);
    }
  }
  return false;  
}
