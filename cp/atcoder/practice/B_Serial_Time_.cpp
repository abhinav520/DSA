...
int dx[] = {1,-1, 0, 0, 0, 0};
int dy[] = {0, 0, 1,-1, 0, 0};
bool isValid(int h, int x, int y, vector<vv<int>> &v, vector<vv<int>> &vis)
{
  return (x>=0 and x<n and y>=0 and y<m and h>=0 and h<k and vis[h][x][y]==0 and v[h][x][y]!=-1 );
}
void dfs(int h, int x, int y, vector<vv<int>> &v, vector<vv<int>> &vis, int &cnt)
{
  vis[h][x][y]=1;
  for(int i=0;i<6;i++){
    // for(int j=0;j<6;j++){
    //   for(int l=0;l<6;l++){
        if(isValid(h+dk[i],x+dx[i],y+dy[i],v,vis)){
          cnt++;
          dfs(h + dk[i], x + dx[i], y + dy[i], v, vis, cnt);
        }
    //   }
    // }
  }
...
void solve(){
  cin>>k>>n>>m;
  vector<vv<int>> v((k), vv<int>( n, vi(m)));
  for(int i=0;i<k;i++){
    for(int j=0;j<n;j++){
      for(int l=0;l<m;l++){
        char c;cin>>c;
  ...
  int x,y;cin>>x>>y;
  x--;y--;
  vector<vv<int>> vis((k), vv<int>( n, vi(m,0)));
  int cnt=1;
  dfs(0,x,y,v,vis,cnt);
  cout<<cnt<<endl;
}
