...
int isSubsetSum(int set[],int n, int sum){
    ...
    if(set[n-1]>sum){
        ...
        return arr[n][sum]=(isSubsetSum(set,n-1,sum-set[n-1]) + isSubsetSum(set,n-1,sum));
    }
}
...
int main()
{
    int set[] = { 3, 34, 4, 12, 1, 2 };
    int sum =4;
    int n = sizeof(set) / sizeof(set[0]);
    if (isSubsetSumDP(set, n, sum) !=0)
        cout<<isSubsetSumDP(set, n, sum)<<(" subset are found with given sum");
    else
